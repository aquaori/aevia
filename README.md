...
# 生产环境可以通过preview命令启动前端
npm run preview

# 再通过start命令启动后端
npm run start

# 但实际生产环境中，不建议使用preview部署前端
# 更推荐使用Nginx等服务来部署，以享受到更快、更安全的Web体验
# 因此，生产环境没有一键启动命令
```
...
- 后端在 `production` 环境下要求显式配置 `JWT_SECRET`，缺失时会拒绝启动。
...
- WebSocket 握手只接受有效的房间会话 token，并通过服务端心跳清理僵尸连接。
...
- 用go重构了后端
- 后端引入消息队列、Actor模型等技术，大幅缓解了高并发场景下的性能表现
- 引入限流、背压、熔断等技术，显著提高高负荷场景的性能水平
- 针对部分极端场景做了性能优化
