...
export const createRoom = async (roomId: string, roomName: string = "Test Room") => {
	const data = await request(`${CONFIG.API_URL}/create-room`, {
		roomId,
		roomName,
		password: CONFIG.ROOM_PASSWORD,
	});
...
export class WebSocketInjector {
	...
	// 持续高频逼真画笔模拟 (用于长时压测)
	// 模拟真实的 60Hz 人类绘图手速
	async injectRealtimeStrokes(
		fps: number = 60,
		durationMs: number = 10000,
		maxPointsPerStroke: number = 200
	) {
		if (!this.ws || !this.isReady) throw new Error("Injector 未就绪");
		...
		for (let frameNum = 0; frameNum < totalFrames; frameNum++) {
			...
			if (!isDrawing) {
				...
				this.ws.send(
					JSON.stringify({
						type: "cmd-start",
						data: { id: currentCmdId, cmd: startObj, lamport: this.lamport },
					})
				);
			} else {
