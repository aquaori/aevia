...
export const runFullRenderSuite = async (
    ...
    page.on('websocket', ws => {
        ws.on('framereceived', frame => {
            const str = typeof frame.payload === 'string' ? frame.payload : frame.payload.toString('utf-8');
    ...
    const renderCompletePromise = new Promise<void>((resolve, reject) => {
        ...
        setTimeout(() => { if (!isRenderFired) reject(new Error('首次渲染等待超时')) }, throttleCpu ? 120000 : 60000);
    });
