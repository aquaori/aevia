...
// ========== 配置区 ==========
const CONFIG = {
	API_URL: process.env.VITE_API_URL || "http://localhost:4646",
	WS_URL: process.env.VITE_WS_URL || "ws://localhost:4646/ws",
...
const joinRoom = async (roomId: string, userName: string) => {
	const data = await request(`${CONFIG.API_URL}/join-room`, {
		roomId,
		userName,
		...
		for (let s = 0; s < strokes; s++) {
			...
			for (let i = 0; i < steps; i++) {
				...
				if (i === 0) {
					...
					const payload = {
						type: "cmd-start",
						data: { id: cmdId, cmd: cmdObj, lamport: this.lamport },
					};
...
const runBenchmarkForScale = async (scale: number, browser: any): Promise<BenchmarkReport> => {
	...
	await request(`${CONFIG.API_URL}/create-room`, {
		roomId,
		roomName: `Bench_${scale}`,
		password: "",
	});
	...
	console.log(
		`  ✅ 灌水完成！真实耗时 ${injectTimeMs.toFixed(0)}ms (TPS: ${(scale / (injectTimeMs / 1000)).toFixed(0)})`
	);
...
(async () => {
	...
	for (const r of results) {
		...
		const sizeStr =
			r.payloadSizeKb > 1024
				? `${(r.payloadSizeKb / 1024).toFixed(2)} MB`
				: `${r.payloadSizeKb.toFixed(1)} KB`;
		...
		console.log(
			`| ${scaleStr.padStart(8)} | ${sizeStr.padStart(10)} | ${renderStr.padStart(10)} | ${totalStr.padStart(13)} | ${tpsStr.padStart(12)} | ${status} |`
		);
	}
	console.log(
		`===================================================================================================`
	...
	console.log(
		`- **用户感知总用时**: 从点击加入房间起，经过网络请求、包结构解析、DOM渲染最终落点上屏的极限体感时长。`
	);
	console.log(
		`- **渲染引擎 TPS**: 渲染纯时长计算出的每秒极值输出能力(Throughput per second)。\n`
	);
