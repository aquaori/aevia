...
onMounted(() => {
    userStore.clearAll();
    axios.get(`${import.meta.env.VITE_API_URL || "http://127.0.0.1:4646"}/get-token-info?token=${token}`)
        .then(res => {
...
const joinRoom = async () => {
    ...
    try {
        await axios.post(`${import.meta.env.VITE_API_URL || "http://127.0.0.1:4646"}/join-room`, {
            roomId: roomId.value,
...
<template>
    <div class="min-h-screen relative flex items-center justify-center p-4 bg-slate-50 overflow-hidden font-sans">
        ...
        <div class="fixed inset-0 overflow-hidden pointer-events-none">
            <div class="absolute -top-[10%] -left-[10%] w-[50vh] h-[50vh] rounded-full mix-blend-multiply filter blur-[80px] opacity-70 bg-purple-300 animate-blob"></div>
            <div class="absolute bottom-[10%] right-[10%] w-[60vh] h-[60vh] rounded-full mix-blend-multiply filter blur-[80px] opacity-70 bg-indigo-200 animate-blob" style="animation-delay: 2s"></div>
        </div>

        <!-- Card -->
        <div class="relative z-10 w-full max-w-[420px]">
            <div class="bg-white/80 backdrop-blur-2xl rounded-[2rem] shadow-2xl shadow-indigo-500/10 border border-white overflow-hidden">
                <!-- Header Decoration -->
                ...
                <div class="p-8 sm:p-10">
                    <div class="text-center mb-8">
                        ...
                        <h1 class="text-2xl font-black text-slate-800 tracking-tight mb-2">邀请你加入</h1>
                        <p class="text-lg font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600">
                            {{ mockRoomName }}
                        </p>
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="group">
                            <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 ml-1">房间密码 (如有)</label>
                            <div class="relative transition-all duration-300 focus-within:transform focus-within:-translate-y-1">
                                <div class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-indigo-500 transition-colors">
                                    <Lock class="w-5 h-5" />
                                </div>
