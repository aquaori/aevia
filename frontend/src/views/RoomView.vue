...
interface Command {
    ...
    lamport: number;         // Lamport时钟
    box: {minX: number, minY: number, maxX: number, maxY: number, width: number, height: number}; // 包围盒
}
...
const remoteSelectionRects = ref<Map<string, { x: number, y: number, w: number, h: number }>>(new Map()); // 远程用户的选框
...
const initialGroupBox = ref<{minX: number, minY: number, maxX: number, maxY: number, width: number, height: number} | null>(null);
 ...
 */
const renderIncrementPoint = (cmd: Command, points: Point[], ctx: CanvasRenderingContext2D, width: number, height: number, skipQueue: boolean = false) => {
...
// 移除未使用的 dpr 声明
    ...
    if (typeof window !== 'undefined' && (window as any).__benchmarkHook) {
        (window as any).__benchmarkHook(cmd.id);
    }
};
...
const renderCanvas = () => {
    ...
    try {
        // 复用通用渲染逻辑
        // renderPageContent 这里不能直接用，因为它内部会 clearRect，而 renderCommand 也会 handle path
        // 实际上 renderPageContent 逻辑就是遍历+渲染，我们也只需要调用它即可
        // 但要注意 renderPageContent 接收的是 Logical Size
        renderPageContent(ctx.value, logicalWidth, logicalHeight, currentPageId.value, [], true);
    } catch (e) {
        console.error('[Render Error]: Failed to render canvas content.', e);
        toast.error('渲染出现错误，部分内容可能无法显示');
    }
};

const flatPoints = (commands: Command[], pageId:number) => {
    const points: FlatPoint[] = [];
    
    commands.forEach(cmd => {
        if (cmd.points && cmd.pageId === pageId) {
            // 扁平化处理：将点集展开，并插入原命令中的信息
            const ptArray: Point[] = Array.isArray(cmd.points) ? cmd.points : Object.values(cmd.points);
            const tool: 'pen' | 'eraser' = cmd.tool ?? 'pen';
            const color: string = cmd.color ?? '#000000';
            ptArray.forEach(pt => {
                points.push({
                    ...pt,
                    cmdId: cmd.id,
    ...
    // 将points按lamport排序，如果相同就比较userId
    const comparePoints = (a: FlatPoint, b: FlatPoint) => {
        const lamportA = Number(a.lamport);
        const lamportB = Number(b.lamport);

        if (lamportA !== lamportB) {
            return lamportA - lamportB;
        }

        if (a.cmdId < b.cmdId) return -1;
        if (a.cmdId > b.cmdId) return 1;
 ...
 */
const pushCommand = (cmdPartial: Partial<Command>, type: "normal" | "start" | "update" | "stop" = "normal") => {
    if (currentCommandIndex.value >= 0) {
        ...
        for (let i = commands.value.length - 1; i >= currentCommandIndex.value; i--) {
            ...
            if (cmd && cmd.userId === userId.value && cmd.pageId === currentPageId.value && cmd.isDeleted) {
                socket.value?.send(JSON.stringify({
                    type: 'delete-cmd',
                    data: {
                        cmdId: cmd.id,
...
const clearClearedCommands = (clearCmd: Command, username = "有用户") => {
    ...
    console.log("commands after specific page clear:", commands.value);
};
...
const getGroupBoundingBox = (cmdIds: Set<string>) => {
    ...
    cmdIds.forEach(id => {
        ...
        if (cmd && !cmd.isDeleted && cmd.pageId === currentPageId.value && cmd.points && cmd.points.length > 0) {
...
const startDrawing = (e: PointerEvent) => {
    ...
    if (currentTool.value === 'cursor') {
        ...
        if (currentGroupBox && selectedCommandIds.value.size > 0) {
            ...
            for (const [key, p] of Object.entries(corners)) {
                if (Math.abs(nx - p.x) <= handleSize && Math.abs(ny - p.y) <= handleSize * (width/height)) {
            ...
            if (!foundHandle) {
                 if (nx >= currentGroupBox.minX && nx <= currentGroupBox.maxX && 
                     ny >= currentGroupBox.minY && ny <= currentGroupBox.maxY) {
                     foundHandle = 'body';
                     actionTarget = 'group';
        ...
        if (actionTarget !== 'group' || (actionTarget === 'group' && foundHandle === 'body')) {
             ...
             for (let i = commands.value.length - 1; i >= 0; i--) {
                const cmd = commands.value[i];
                if(!cmd) continue;
                if (cmd.isDeleted || cmd.pageId !== currentPageId.value || cmd.type !== 'path') continue;
                const box = getCommandBoundingBox(cmd);
                if (!box) continue;
                const buffer = 10 / width;
                if (nx >= box.minX - buffer && nx <= box.maxX + buffer &&
                    ny >= box.minY - buffer && ny <= box.maxY + buffer) {
                    hitCmdId = cmd.id;
                    break;
                }
             }
    ...
    // 橡皮擦不需要压感效果
    lastWidth = currentTool.value === 'eraser' ? currentSize.value : currentSize.value * (initialPressure * 2);
        ...
        selectedCommandIds.value.forEach(cmdId => {
            ...
            if (activeTransformHandle.value === 'body') {
                ...
                switch (activeTransformHandle.value) {
                    case 'tl': anchorX = initialBox.maxX; anchorY = initialBox.maxY; break;
                    case 'tr': anchorX = initialBox.minX; anchorY = initialBox.maxY; break;
                    case 'bl': anchorX = initialBox.maxX; anchorY = initialBox.minY; break;
                    case 'br': anchorX = initialBox.minX; anchorY = initialBox.minY; break;
                }
                ...
                switch (activeTransformHandle.value) {
                    case 'tl': originalW = initialBox.minX - initialBox.maxX; originalH = initialBox.minY - initialBox.maxY; break;
                    case 'tr': originalW = initialBox.maxX - initialBox.minX; originalH = initialBox.minY - initialBox.maxY; break;
                    case 'bl': originalW = initialBox.minX - initialBox.maxX; originalH = initialBox.maxY - initialBox.minY; break;
                    case 'br': originalW = initialBox.maxX - initialBox.minX; originalH = initialBox.maxY - initialBox.minY; break;
                }
...
function updateMousePosition() {
        ...
        if (interactionMode.value === 'dragging' && selectedCommandIds.value.size > 0 && dragStartPos.value) {
            // 计算自上次发送以来的增量 (Delta)
...
const stopDrawing = (e: PointerEvent) => {
    if (isDrawing.value) {
        ...
        if (cmd && cmd.points && cmd.points.length > 0) {
            cmd.box = getCommandBoundingBox(cmd) ?? {minX: 0, minY: 0, maxX: 0, maxY: 0, width: 0, height: 0};
        }
        // 发送剩余点
        pushCommand({
            id: currentDrawingId.value || undefined,
        ...
        if (currentTool.value === 'cursor') {
            ...
            if (interactionMode.value === 'box-selecting' && selectionRect.value) {
                ...
                commands.value.forEach(cmd => {
                    if (cmd.isDeleted || cmd.pageId !== currentPageId.value || cmd.type !== 'path') return;
                    const box = getCommandBoundingBox(cmd);
                    if (!box) return;
                    ...
                    // 只要两个AABB有重叠即可
                    if (box.minX < rectMaxX && box.maxX > rectMinX &&
                        box.minY < rectMaxY && box.maxY > rectMinY && !cmd.isDeleted && cmd.tool === 'pen') {
            ...
            else if ((interactionMode.value === 'dragging' || interactionMode.value === 'resizing') 
                    && selectedCommandIds.value.size > 0 && dragStartPos.value) {
                    
                // 无论是移动还是缩放，结束时都统一发送全量 Points 
                // 这解决了实时增量导致的偏移累积问题，并方便后端直接存库
                const updates = Array.from(selectedCommandIds.value).map(id => {
                    const cmd = commands.value.find(c => c.id === id);
...
const copyLink = async () => {
    ...
    axios.get(`${apiUrl}/generate-share-token?roomId=${roomId.value}`)
        .then(res => {
        ...
        .catch(err => {
            ...
            console.error("Error generating share token: ", err.response?.data?.msg || "未知错误");
        });
};
 ...
 * @param pageId 页面 ID
 */
const renderPageContent = (targetCtx: CanvasRenderingContext2D, width: number, height: number, pageId: number, commandsQueue: Command[] = [], skipQueue: boolean = false, isDirtyRender: boolean = false) => {
    const commandList = commandsQueue.length > 0 ? commandsQueue : commands.value;
    const _renderStart = performance.now();
    targetCtx.clearRect(0, 0, width, height);
    targetCtx.save();
    const flattedPoints: FlatPoint[] = flatPoints(commandList, pageId);
    const suspendedPoints: Map<string, FlatPoint> = new Map();
    let currentBatchPoints: FlatPoint[] = [];
    let activeCmdId: string | null = null;
    let activeCmd: Command | null = null;

    const flushBatch = () => {
        if (activeCmdId && activeCmd && currentBatchPoints.length > 0) {
            renderIncrementPoint(activeCmd, currentBatchPoints, targetCtx, width, height, skipQueue);
            suspendedPoints.set(activeCmdId, currentBatchPoints[currentBatchPoints.length - 1]!);
            currentBatchPoints = [];
        }
    };

    flattedPoints.forEach(point => {

        // 1. 如果换了命令，或者这是第一个点，先提交上一波的渲染
        if (point.cmdId !== activeCmdId) {
            flushBatch();
            
            activeCmdId = point.cmdId;
            // 构造一个临时的虚拟 Command 对象用于渲染引擎识别颜色、工具等基础属性
            activeCmd = {
                id: point.cmdId,
                userId: point.userId,
                color: point.color,
                tool: point.tool,
                size: point.size,
                points: [], // renderIncrementPoint 会接手绘制剩下的逻辑
            ...
            // 2. 衔接逻辑：如果是该命令后续进来的点，需要把上一次悬挂的点作为起点加入当前批次
            const lastPoint = suspendedPoints.get(point.cmdId);
            if (lastPoint) {
                currentBatchPoints.push(lastPoint);
            }
        }
    ...
    console.log(`${logPrefix} 点数=${flattedPoints.length} 耗时=${(_renderEnd - _renderStart).toFixed(2)}ms`);
};
    ...
    if (selectedCommandIds.value.size > 0) {
        ...
        if (groupBox) {
            ...
            corners.forEach(p => {
                ...
                uiCtx.value!.rect(p.x - handleSize/2, p.y - handleSize/2, handleSize, handleSize);
                uiCtx.value!.fill();
                uiCtx.value!.stroke();
            });
...
const renderPreviewCanvas = (el: HTMLCanvasElement | any, index: number) => {
    ...
    requestAnimationFrame(() => {
        ...
        // 使用逻辑尺寸进行渲染
        renderPageContent(ctx, rect.width, rect.height, index, [], true);
    });
};
...
const handleKeydown = (e: KeyboardEvent) => {
    ...
    if (e.ctrlKey || e.metaKey) {
        ...
        switch(e.key.toLowerCase()) {
            case 'p': setTool('pen'); break;   // P: 切换画笔
            case 'e': setTool('eraser'); break;// E: 切换橡皮
            case 'c': activeMenu.value = 'color'; break; // C: 打开颜色盘
            case '?': showShortcuts.value = !showShortcuts.value; break; // ?: 显示快捷键指南
            case 'f': toggleFullscreen(); break; // F: 全屏
        }
    }
};
    ...
    try {
        ...
        socket.value.onmessage = (event) => {
            try {
                ...
                if (JSON.parse(event.data)) {
                    ...
                    if (msg.type === 'init') {
                        ...
                        if(initData.commands.length > 0) {
                            ...
                            const lastPoint = lastCommand.points && Object.keys(lastCommand.points).length > 0 
                                ? lastCommand.points[Object.keys(lastCommand.points).length - 1] 
                                : null;
                            if(lastPoint) {
                                const maxLamport = lastPoint.lamport;
                                useLamportStore().lamport = Math.max(useLamportStore().lamport, maxLamport);
                            }
                        }
                        ...
                        const { userId: changedUserId, userName: changedUserName, type: type } = msg.data;
                        ...
                        if(pushType == 'normal' || pushType == 'start') {
                            ...
                            if (!isDrawing.value && pushType == "normal") {
                                if(msg.data.lamport) {
                                ...
                                renderCanvas();
                            } else if (!isDrawing.value && pushType == "start") {
                                if(msg.data.lamport) {
                                    useLamportStore().syncLamport(msg.data.lamport);
                                }
                            ...
                            const localCmd = commands.value.find((c: Command) => c.id === cmd.id);
                            if(localCmd) {
                                // 更新本地命令的points属性
                                localCmd.points = [...(localCmd.points || []), ...points];
                            } else {
                                // 如果找不到本地命令，说明可能是新命令，添加到命令数组
                                insertCommand(cmd);
                            }
                            if(cmd.pageId != currentPageId.value) {
                                return;
                            ...
                            // 快速画线时，最后一批 pendingPoints 会随 stop 消息一起发送，
                            // 如果不合并这些点，其他客户端的线段会短一截
                            const stopPoints = msg.data.points ?? msg.data.cmd?.points ?? [];
                            const localCmd = commands.value.find((c: Command) => c.id === cmdId);
                            ...
                            if (localCmd) {
                                if (stopPoints.length > 0) {
                                    ...
                                    if (canvasRef.value && ctx.value) {
                                        ...
                                        const logicalWidth = canvasRef.value.width / dpr;
                                        const logicalHeight = canvasRef.value.height / dpr;
                                        renderIncrementPoint(localCmd, stopPoints, ctx.value, logicalWidth, logicalHeight);
                                    }
                                }
                            ...
                            if (localCmd && localCmd.type === 'path' && localCmd.points && localCmd.points.length === 1) {
                                if(localCmd.pageId != currentPageId.value) {
                                    return;
                                }
                                if (canvasRef.value && ctx.value) {
                                    ...
                                    const color = localCmd.tool === 'eraser' ? '#ffffff' : (localCmd.color || '#000000');
                                    const op = localCmd.tool === 'eraser' ? 'destination-out' : 'source-over';
                ...
                console.error('[WebSocket Message Error]: Failed to parse or process message.', error, event.data);
            }
        };
...
const reRenderDirtyRect = (dirtyRect: any) => {
    ...
    renderPageContent(ctx.value, canvasW, canvasH, currentPageId.value, commandList, true, true);
    ctx.value.restore();
};
...
<template>
    <div class="fixed inset-0 overflow-hidden bg-slate-50 touch-none select-none">
        <!-- Name Prompt -->
        <div v-if="showNamePrompt" class="fixed inset-0 z-100 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl">
                <h3 class="text-xl font-bold mb-4">请输入你的名字</h3>
                <input v-model="newName" @keyup.enter="saveName" class="w-full px-4 py-2 border rounded-xl mb-4 focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="你的名字" />
                <button @click="saveName" class="w-full py-2 bg-indigo-600 text-white rounded-xl font-medium">开始绘画</button>
            </div>
        </div>

        <!-- Reconnecting Overlay -->
        <div v-if="isReconnecting" class="fixed inset-0 z-100 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-2 sm:p-4">
             <div class="bg-white rounded-2xl p-4 sm:p-8 max-h-full overflow-y-auto max-w-sm w-full shadow-2xl flex flex-col items-center text-center">
                 <RotateCw class="w-8 h-8 sm:w-12 sm:h-12 text-indigo-500 animate-[spin_2s_linear_infinite] mb-2 sm:mb-4" />
                 ...
                 <div class="w-full bg-slate-100 rounded-full h-1.5 sm:h-2 mb-2 overflow-hidden items-start flex">
                     <div class="bg-indigo-500 h-1.5 sm:h-2 rounded-full transition-all duration-300" :style="{ width: (reconnectCount / MAX_RECONNECT * 100) + '%' }"></div>
                 </div>
                 <p class="text-xs sm:text-sm font-medium text-slate-400">第 {{ reconnectCount }} / {{ MAX_RECONNECT }} 次尝试</p>
             </div>
        </div>

        <!-- Reconnect Failed Overlay -->
        <div v-if="reconnectFailed" class="fixed inset-0 z-100 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-2 sm:p-4">
             <div class="bg-white rounded-2xl p-4 sm:p-8 max-h-full overflow-y-auto max-w-sm w-full shadow-2xl flex flex-col items-center text-center pointer-events-auto">
                 <div class="w-12 h-12 sm:w-16 sm:h-16 bg-red-50 text-red-500 rounded-full flex items-center justify-center mb-3 sm:mb-6">
                     <X class="w-6 h-6 sm:w-8 sm:h-8" />
                 </div>
                 <h3 class="text-lg sm:text-xl font-bold text-slate-800 mb-1 sm:mb-2">连接失败</h3>
                 <p class="text-slate-500 mb-4 sm:mb-6 text-xs sm:text-base">服务器连接超时，请返回首页或重新尝试连接。</p>
                 <div class="flex gap-2 sm:gap-3 w-full">
                     <button @click="router.push('/')" class="flex-1 py-2 sm:py-3 px-2 sm:px-4 text-sm sm:text-base bg-slate-100 text-slate-700 hover:bg-slate-200 font-bold rounded-xl transition-colors">
                         返回首页
                     </button>
                     <button @click="retryReconnect" class="flex-1 py-2 sm:py-3 px-2 sm:px-4 text-sm sm:text-base bg-indigo-600 text-white hover:bg-indigo-700 font-bold rounded-xl transition-colors">
                         重试连接
                     </button>
                 </div>
             </div>
        ...
        >
            ...
            >
                <div class="bg-white/80 backdrop-blur-md rounded-3xl p-6 shadow-2xl border border-white/50 flex flex-col items-center gap-4">
                    <div 
                    ...
                    ></div>
                    <div class="text-slate-500 font-bold font-mono text-lg">{{ currentSize }}px</div>
                </div>
            </div>
        </transition>
        ...
        >
            ...
            >
                <div class="bg-white/90 backdrop-blur-sm rounded-4xl shadow-2xl w-full max-w-md max-h-[85vh] overflow-y-auto border border-white/50 ring-1 ring-white/50 scrollbar-hide">
                    <!-- Header -->
                    <div class="sticky top-0 z-10 flex justify-between items-start px-4 hmd:px-6 py-3 hmd:py-4 bg-white/80 backdrop-blur-xl border-b border-slate-100/50">
                        <div>
                            <h3 class="text-lg hmd:text-xl font-black text-slate-800 tracking-tight flex items-center gap-2">
                                <Keyboard class="w-4 h-4 hmd:w-5 hmd:h-5 text-indigo-500" /> 快捷指令
                                <span class="text-[10px] hmd:text-xs font-medium text-slate-400 opacity-80 animate-pulse">(Scrollable)</span>
                            </h3>
                            <p class="hidden hmd:block text-slate-400 text-[11px] hmd:text-xs mt-0.5 font-medium">高效创作必备</p>
                        </div>
                        <button 
                    ...
                    <div class="p-3 hmd:p-4 grid grid-cols-2 gap-2 hmd:gap-3">
                        <div class="p-2 hmd:p-3 rounded-xl hmd:rounded-2xl bg-white shadow-sm border border-slate-100 hover:border-indigo-100 hover:shadow-md transition-all group">
                            <div class="flex items-center gap-2 mb-1">
                                <div class="w-6 h-6 hmd:w-7 h-7 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center group-hover:scale-110 transition-transform">
                                    <Pencil class="w-3 h-3 hmd:w-3.5 h-3.5" />
                                </div>
                                <span class="font-bold text-slate-700 text-xs hmd:text-sm">画笔</span>
                            </div>
                            <div class="flex justify-end">
                                <kbd class="px-1.5 py-0.5 bg-slate-100 rounded-lg border-b-2 border-slate-200 font-mono text-[10px] hmd:text-xs font-bold text-slate-500 group-hover:bg-white group-hover:border-indigo-200 group-hover:text-indigo-500 transition-colors">P</kbd>
                            </div>
                        </div>

                        <div class="p-2 hmd:p-3 rounded-xl hmd:rounded-2xl bg-white shadow-sm border border-slate-100 hover:border-pink-100 hover:shadow-md transition-all group">
                            <div class="flex items-center gap-2 mb-1">
                                <div class="w-6 h-6 hmd:w-7 h-7 rounded-lg bg-pink-50 text-pink-600 flex items-center justify-center group-hover:scale-110 transition-transform">
                                    <Eraser class="w-3 h-3 hmd:w-3.5 h-3.5" />
                                </div>
                                <span class="font-bold text-slate-700 text-xs hmd:text-sm">橡皮擦</span>
                            </div>
                            <div class="flex justify-end">
                                <kbd class="px-1.5 py-0.5 bg-slate-100 rounded-lg border-b-2 border-slate-200 font-mono text-[10px] hmd:text-xs font-bold text-slate-500 group-hover:bg-white group-hover:border-pink-200 group-hover:text-pink-500 transition-colors">E</kbd>
                            </div>
                        </div>

                        <div class="p-2 hmd:p-3 rounded-xl hmd:rounded-2xl bg-white shadow-sm border border-slate-100 hover:border-orange-100 hover:shadow-md transition-all group">
                            <div class="flex items-center gap-2 mb-1">
                                <div class="w-6 h-6 hmd:w-7 h-7 rounded-lg bg-orange-50 text-orange-600 flex items-center justify-center group-hover:scale-110 transition-transform">
                                    <Palette class="w-3 h-3 hmd:w-3.5 h-3.5" />
                                </div>
                                <span class="font-bold text-slate-700 text-xs hmd:text-sm">调色板</span>
                            </div>
                            <div class="flex justify-end">
                                <kbd class="px-1.5 py-0.5 bg-slate-100 rounded-lg border-b-2 border-slate-200 font-mono text-[10px] hmd:text-xs font-bold text-slate-500 group-hover:bg-white group-hover:border-orange-200 group-hover:text-orange-500 transition-colors">C</kbd>
                            </div>
                        </div>

                         <div class="p-2 hmd:p-3 rounded-xl hmd:rounded-2xl bg-white shadow-sm border border-slate-100 hover:border-blue-100 hover:shadow-md transition-all group">
                            <div class="flex items-center gap-2 mb-1">
                                <div class="w-6 h-6 hmd:w-7 h-7 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center group-hover:scale-110 transition-transform">
                                    <Maximize class="w-3 h-3 hmd:w-3.5 h-3.5" />
                                </div>
                                <span class="font-bold text-slate-700 text-xs hmd:text-sm">全屏</span>
                            </div>
                            <div class="flex justify-end">
                                <kbd class="px-1.5 py-0.5 bg-slate-100 rounded-lg border-b-2 border-slate-200 font-mono text-[10px] hmd:text-xs font-bold text-slate-500 group-hover:bg-white group-hover:border-blue-200 group-hover:text-blue-500 transition-colors">F</kbd>
                            </div>
                        </div>

                        <div class="p-2 hmd:p-3 rounded-xl hmd:rounded-2xl bg-white shadow-sm border border-slate-100 hover:border-slate-200 hover:shadow-md transition-all group">
                            <div class="flex items-center gap-2 mb-1">
                                <div class="w-6 h-6 hmd:w-7 h-7 rounded-lg bg-slate-100 text-slate-600 flex items-center justify-center group-hover:scale-110 transition-transform">
                                    <RotateCcw class="w-3 h-3 hmd:w-3.5 h-3.5" />
                                </div>
                                <span class="font-bold text-slate-700 text-xs hmd:text-sm">撤销</span>
                            </div>
                            <div class="flex justify-end gap-1">
                                <kbd class="px-1 py-0.5 bg-slate-100 rounded-md border-b-2 border-slate-200 font-mono text-[10px] font-bold text-slate-500 group-hover:bg-white group-hover:border-slate-300 group-hover:text-slate-700 transition-colors">Ctrl+Z</kbd>
                            </div>
                        </div>
                        ...
                        <div class="p-2 hmd:p-3 rounded-xl hmd:rounded-2xl bg-white shadow-sm border border-slate-100 hover:border-slate-200 hover:shadow-md transition-all group">
                            <div class="flex items-center gap-2 mb-1">
                                <div class="w-6 h-6 hmd:w-7 h-7 rounded-lg bg-slate-100 text-slate-600 flex items-center justify-center group-hover:scale-110 transition-transform">
                                    <RotateCw class="w-3 h-3 hmd:w-3.5 h-3.5" />
                                </div>
                                <span class="font-bold text-slate-700 text-xs hmd:text-sm">重做</span>
                            </div>
                            <div class="flex justify-end gap-1">
                                <kbd class="px-1 py-0.5 bg-slate-100 rounded-md border-b-2 border-slate-200 font-mono text-[10px] font-bold text-slate-500 group-hover:bg-white group-hover:border-slate-300 group-hover:text-slate-700 transition-colors">Ctrl+Y</kbd>
                            </div>
                        </div>
                    </div>
                    
                    <div class="px-4 hmd:px-6 py-2 hmd:py-3 bg-slate-50 border-t border-slate-100 text-center sticky bottom-0 z-10 hidden hsm:block">
                        <button 
                            @click="showShortcuts = false"
        ...
        >
            ...
            >
                ...
                <div class="flex justify-between items-center mb-4 hmd:mb-8">
                     <div>
                        <h3 class="text-xl hmd:text-3xl font-black text-slate-800 tracking-tight flex items-center gap-2 hmd:gap-3">
                            <LayoutGrid class="w-6 h-6 hmd:w-8 h-8 text-indigo-500" /> 总览视图
                        </h3>
                        <p class="text-slate-500 font-medium mt-1 hmd:mt-2 ml-1 text-xs hmd:text-sm">总计 {{ totalPages }} 张画布</p>
                    </div>
                    <button 
                    ...
                    >
                        <X class="w-5 h-5 hmd:w-6 h-6" />
                    </button>
                </div>

                <!-- Grid -->
                <div class="flex-1 overflow-y-auto min-h-0 scrollbar-hide pb-20">
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 hmd:gap-6 p-2 hmd:p-4">
                        <div 
                            ...
                            :class="currentPageId === i - 1 ? 'border-indigo-500 ring-4 ring-indigo-500/20 shadow-xl scale-[1.02]' : 'border-transparent hover:border-slate-300 hover:shadow-md hover:-translate-y-1'"
                        >
                            <!-- Preview Canvas -->
                            <div class="absolute inset-1 hmd:inset-2 bg-slate-50/50 rounded-lg hmd:rounded-xl overflow-hidden border border-slate-100">
                                <canvas 
                            ...
                            <!-- Page Number Label -->
                            <div class="absolute bottom-2 hmd:bottom-4 left-2 hmd:left-4 px-2 hmd:px-3 py-0.5 hmd:py-1 bg-white/90 backdrop-blur-sm rounded-md hmd:rounded-lg shadow-sm border border-slate-200 text-[10px] hmd:text-xs font-bold text-slate-600 font-mono">
                                {{ i }}
                            </div>

                            <!-- Current Indicator -->
                            <div v-if="currentPageId === i - 1" class="absolute top-2 hmd:top-4 right-2 hmd:right-4 w-2 h-2 hmd:w-3 hmd:h-3 bg-indigo-500 rounded-full shadow-sm ring-2 ring-white"></div>
                            
                            <!-- 跨页状态探针 (Cross-page Activity Heatmap) -->
                            <div class="absolute bottom-2 hmd:bottom-4 right-2 hmd:right-4 flex -space-x-1.5 hmd:-space-x-2">
                                <template v-for="(cursor, key) in Array.from(remoteCursors.values()).filter(c => c.pageId === i - 1).slice(0, 3)" :key="cursor.userId">
                                    <div 
                                ...
                                <!-- 超出3个人的气泡 -->
                                <div v-if="Array.from(remoteCursors.values()).filter(c => c.pageId === i - 1).length > 3" 
                        ...
                        <div 
                            @click="nextPage(); goToPage(totalPages - 1)" 
                        ...
                        >
                            <div class="w-8 h-8 hmd:w-12 hmd:h-12 rounded-full bg-white shadow-sm border border-slate-200 flex items-center justify-center group-hover:scale-110 transition-transform">
        ...
        >
            <div v-if="showMemberList" class="fixed top-0 right-0 bottom-0 w-64 hmd:w-72 bg-white/95 backdrop-blur-md shadow-2xl border-l border-slate-100 z-[80] flex flex-col pointer-events-auto">
                <div class="flex items-center justify-between p-3 border-b border-slate-100 shrink-0">
                    <h3 class="text-sm font-bold text-slate-800 flex items-center gap-2">
                        ...
                        在线协作成员
                        <span class="px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-600 text-[10px] font-bold">{{ onlineCount }}</span>
                    </h3>
                    <button @click="showMemberList = false" class="p-1.5 -mr-1.5 rounded-full hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-colors">
                        <X class="w-4 h-4" />
                    </button>
                </div>
                <!-- 成员列表滚动区 -->
                <div class="flex-1 overflow-y-auto p-3 space-y-1.5 min-h-0 scrollbar-hide">
                    <div v-if="memberList.length === 0" class="flex flex-col items-center justify-center py-8 text-slate-400 gap-2">
                    ...
                    <div v-else v-for="(member, index) in memberList" :key="index" class="flex items-center gap-2.5 p-2 rounded-xl hover:bg-slate-50 border border-transparent hover:border-slate-100 transition-colors group">
                        <!-- 用户头像 (首字母) + 在线小绿点 -->
                        <div class="relative w-8 h-8 shrink-0 rounded-full bg-linear-to-br from-indigo-100 to-purple-100 text-indigo-600 flex items-center justify-center font-black text-xs shadow-sm shadow-indigo-100/50 border border-indigo-50 group-hover:scale-105 transition-transform duration-300">
                        ...
                        <div class="flex-1 min-w-0 pr-1">
                            ...
                            <div class="font-bold text-slate-700 text-[13px] truncate flex items-center gap-1.5">
                                ...
                                <span v-if="member[1] === (Array.isArray(username) ? username[0] : username)" class="px-1.5 py-[1px] rounded-[4px] shrink-0 text-[9px] items-center bg-emerald-50 text-emerald-600 font-bold tracking-widest border border-emerald-100">我</span>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <!-- Rotate Hint -->
        <div class="hidden md:hidden portrait:flex fixed inset-0 z-100 bg-slate-900/95 text-white items-center justify-center flex-col p-8 text-center backdrop-blur-sm">
        ...
        <transition enter-active-class="transition duration-300" enter-from-class="-translate-y-full opacity-0" enter-to-class="translate-y-0 opacity-100" leave-active-class="transition duration-300" leave-from-class="translate-y-0 opacity-100" leave-to-class="-translate-y-full opacity-0">
            <div v-if="!isFullscreen" class="absolute top-0 left-0 right-0 h-16 px-4 flex justify-between items-center z-10 pointer-events-none">
                
                <!-- Left: Mock Room Name -->
                <div class="pointer-events-auto flex items-center gap-1 hmd:gap-3 bg-white/80 backdrop-blur-md shadow-sm border border-white/50 px-2 hmd:px-2 py-1.5 hmd:py-2 rounded-xl hmd:rounded-2xl">
                    <div class="w-6 h-6 hmd:w-8 hmd:h-8 rounded-lg bg-linear-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white shadow-md">
                        <Monitor class="w-3 h-3 hmd:w-5 hmd:h-5" />
                    </div>
                    <div>
                        <div class="font-bold text-slate-800 text-sm hmd:text-base leading-tight">{{ roomName }}</div>
                        <div class="hidden hmd:block text-[10px] text-slate-400 font-mono font-medium tracking-wider">ID: {{ roomId }}</div>
                    </div>
                </div>
                    ...
                    <div class="flex items-center">
                        <div class="relative z-20">
                            ...
                            <button 
                                ...
                                :class="{'ring-4 ring-indigo-500/10 border-indigo-300 bg-white': activeMenu === 'more'}"
                            >
                                <div class="relative flex items-center justify-center">
                                    <div class="w-6 h-6 hmd:w-7 h-7 rounded-lg hmd:rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center group-hover:bg-indigo-100 transition-colors"
                                         :class="{'bg-indigo-500 text-white group-hover:bg-indigo-600': activeMenu === 'more'}">
                                    ...
                                    <span class="absolute -top-0.5 -right-0.5 flex h-2 w-2 hmd:h-2.5 hmd:w-2.5">
                                        ...
                                        <span class="relative inline-flex rounded-full h-2 w-2 hmd:h-2.5 hmd:w-2.5 bg-emerald-500 border border-white"></span>
                                    </span>
                                </div>
                                <span class="text-[11px] hmd:text-xs font-bold text-slate-700 group-hover:text-indigo-600 transition-colors">
                                    {{ onlineCount }} <span class="hidden hmd:inline">人协作中</span>
                                </span>
                            </button>
                            ...
                            <transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 scale-95 translate-y-2" enter-to-class="opacity-100 scale-100 translate-y-0" leave-active-class="transition duration-150 ease-in" leave-from-class="opacity-100 scale-100 translate-y-0" leave-to-class="opacity-0 scale-95 translate-y-2">
                                <div v-if="activeMenu === 'more'" class="absolute top-full right-0 mt-3 w-56 hmd:w-64 bg-white/95 backdrop-blur-sm border border-white/20 rounded-2xl shadow-2xl overflow-hidden origin-top-right p-1.5 flex flex-col gap-1 z-50">
                                    
                                    <button @click="copyLink" class="flex items-center gap-2 hmd:gap-3 w-full p-1.5 hmd:p-3 rounded-xl hover:bg-indigo-50 group text-left transition-colors">
                                        <div class="w-6 h-6 hmd:w-8 hmd:h-8 rounded-lg bg-indigo-100 text-indigo-600 flex items-center justify-center group-hover:bg-indigo-200 transition-colors shrink-0">
                                            <Check v-if="hasCopied" class="w-3 h-3 hmd:w-4 hmd:h-4" />
                                            <Copy v-else class="w-3 h-3 hmd:w-4 hmd:h-4" />
                                        </div>
                                        <div class="flex-1">
                                            <div class="text-[11px] hmd:text-sm font-bold text-slate-700">{{ hasCopied ? '已复制链接' : '复制邀请链接' }}</div>
                                            <div class="hidden hmd:block text-[10px] text-slate-400">点击复制房间地址</div>
                                        </div>
                                    </button>
                                    
                                    <div class="h-px bg-slate-100 mx-2 my-0.5"></div>

                                    <button @click="openMemberList" class="flex items-center gap-2 hmd:gap-3 w-full p-1.5 hmd:p-3 rounded-xl hover:bg-slate-50 text-left transition-colors">
                                        <div class="w-6 h-6 hmd:w-8 hmd:h-8 rounded-lg bg-slate-100 text-slate-500 flex items-center justify-center shrink-0">
                                            <Users class="w-3 h-3 hmd:w-4 hmd:h-4" />
                                        </div>
                                        <div class="flex-1">
                                            <div class="text-[11px] hmd:text-sm font-bold text-slate-700">在线人数</div>
                                            <div class="hidden hmd:block text-[10px] text-slate-400">{{ onlineCount }} 人正在协作</div>
                                        </div>
                                    </button>

                                    <button @click="showShortcuts = !showShortcuts; activeMenu = null" class="flex items-center gap-2 hmd:gap-3 w-full p-1.5 hmd:p-3 rounded-xl hover:bg-slate-50 text-left transition-colors">
                                        <div class="w-6 h-6 hmd:w-8 hmd:h-8 rounded-lg bg-slate-100 text-slate-500 flex items-center justify-center shrink-0">
                                            <Keyboard class="w-3 h-3 hmd:w-4 hmd:h-4" />
                                        </div>
                                        <div class="flex-1">
                                            <div class="text-[11px] hmd:text-sm font-bold text-slate-700">快捷键指南</div>
                                            <div class="hidden hmd:block text-[10px] text-slate-400">查看常用快捷操作</div>
                                        </div>
                                    </button>
                                </div>
                            </transition>
        ...
        <div class="fixed bottom-3 hmd:bottom-4 right-3 hmd:right-4 z-50 flex items-center gap-0.5 hmd:gap-1 bg-white/90 backdrop-blur-sm border border-slate-200/60 p-1 hmd:p-1.5 rounded-xl shadow-lg ring-1 ring-slate-100 touch-none select-none">
            <button 
                @click="prevPage" 
            ...
            <button 
                ...
                title="页面概览"
            >
                <LayoutGrid v-if="showPageOverview" class="w-3.5 h-3.5 hmd:w-4 hmd:h-4 text-indigo-500" />
                <template v-else>
            ...
            <button 
                ...
                :class="currentPageId === totalPages - 1 ? 'text-indigo-500 hover:bg-indigo-50' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-700'"
                :title="currentPageId === totalPages - 1 ? '新建页面' : '下一页'"
            >
                <Plus v-if="currentPageId === totalPages - 1" class="w-4 h-4 hmd:w-5 hmd:h-5 group-hover:scale-110 transition-transform" />
        ...
        <div class="fixed inset-0 w-full h-full touch-none select-none overflow-hidden bg-white">
            ...
            <canvas
                ...
                :class="{'cursor-none': currentTool === 'eraser', 'cursor-crosshair': currentTool === 'pen', 'cursor-default': currentTool === 'cursor'}"
            ></canvas>
        ...
        >
             <transition name="toolbar-pop" mode="out-in">
                 ...
                 <!-- ================= FULL TOOLBAR ================= -->
                 <div v-if="!isToolbarCollapsed" key="expanded" class="pointer-events-auto bg-white/80 backdrop-blur-sm shadow-[0_8px_32px_rgba(0,0,0,0.12)] border border-white/40 p-1 hmd:p-2 rounded-2xl flex items-center gap-1 hmd:gap-2">
                     ...
                     <div class="relative shrink-0">
                         <button @click="setTool('cursor')" class="p-2 hmd:p-3 cursor-pointer rounded-xl transition-all" :class="currentTool === 'cursor' ? 'bg-indigo-50 text-indigo-600 shadow-sm ring-1 ring-indigo-200' : 'text-slate-400 hover:bg-slate-50'">
                     ...
                     <!-- Pen -->
                     <div class="relative shrink-0">
                         <transition enter-active-class="transition duration-200" enter-from-class="opacity-0 translate-y-4 scale-95" enter-to-class="opacity-100 translate-y-0 scale-100" leave-active-class="transition duration-150" leave-from-class="opacity-100 translate-y-0 scale-100" leave-to-class="opacity-0 translate-y-4 scale-95">
                             <div v-if="activeMenu === 'pen'" class="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 p-3 hmd:p-4 bg-white/90 backdrop-blur-sm border border-white rounded-xl shadow-xl w-40 hmd:w-48 origin-bottom">
                                 <div class="text-[10px] hmd:text-xs font-bold text-slate-400 mb-1.5 hmd:mb-2 flex justify-between">
                                     <span>画笔粗细</span>
                         ...
                         <button @click="toggleMenu('pen')" class="p-2 hmd:p-3 cursor-pointer rounded-xl transition-all relative z-10" :class="currentTool === 'pen' ? 'bg-indigo-50 text-indigo-600 shadow-sm ring-1 ring-indigo-200' : 'text-slate-400 hover:bg-slate-50'">
                     ...
                     <div class="relative shrink-0">
                         <transition enter-active-class="transition duration-200" enter-from-class="opacity-0 translate-y-4 scale-95" enter-to-class="opacity-100 translate-y-0 scale-100" leave-active-class="transition duration-150" leave-from-class="opacity-100 translate-y-0 scale-100" leave-to-class="opacity-0 translate-y-4 scale-95">
                             <div v-if="activeMenu === 'eraser'" class="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 p-3 hmd:p-4 bg-white/90 backdrop-blur-sm border border-white rounded-xl shadow-xl w-48 hmd:w-56 origin-bottom">
                                 <div class="space-y-3 hmd:space-y-4">
                                     ...
                                     <div ref="sliderTrackRef" @mousedown="handleSliderStart" @touchstart.passive="handleSliderStart" class="slider-track relative h-8 hmd:h-10 bg-slate-100 rounded-lg flex items-center p-1 cursor-pointer overflow-hidden select-none">
                                         <div class="absolute inset-0 flex items-center justify-center text-[9px] hmd:text-[10px] text-slate-400 font-bold uppercase tracking-wider">滑动清空画布</div>
                                         <div class="absolute top-1 bottom-1 w-10 hmd:w-12 bg-white rounded-md shadow-sm border border-slate-200 flex items-center justify-center text-red-500 z-10" :style="{ transform: 'translateX(' + sliderX + 'px)' }">
                         ...
                         <button @click="toggleMenu('eraser')" class="p-2 hmd:p-3 cursor-pointer rounded-xl transition-all relative z-10" :class="currentTool === 'eraser' ? 'bg-indigo-50 text-indigo-600 shadow-sm ring-1 ring-indigo-200' : 'text-slate-400 hover:bg-slate-50'">
                     ...
                     <div class="relative shrink-0">
                         <transition enter-active-class="transition duration-200" enter-from-class="opacity-0 translate-y-4 scale-95" enter-to-class="opacity-100 translate-y-0 scale-100" leave-active-class="transition duration-150" leave-from-class="opacity-100 translate-y-0 scale-100" leave-to-class="opacity-0 translate-y-4 scale-95">
                             <div v-if="activeMenu === 'color'" class="absolute bottom-full left-1/2 -translate-x-1/2 mb-4 p-2 hmd:p-3 bg-white/90 backdrop-blur-sm border border-white rounded-xl shadow-xl w-48 hmd:w-64 origin-bottom grid grid-cols-6 gap-1.5 hmd:gap-2">
                                 <button v-for="c in colors" :key="c" @click="setColor(c)" class="w-6 h-6 cursor-pointer hmd:w-8 hmd:h-8 rounded-full border border-slate-100 shadow-sm hover:scale-110 transition-transform relative" :style="{ backgroundColor: c }">
                                     <div v-if="currentColor === c" class="absolute inset-0 flex items-center justify-center">
                                         <div class="w-2 h-2 hmd:w-2.5 hmd:h-2.5 bg-white rounded-full shadow-sm" :class="{'bg-black': c === '#ffffff'}"></div>
                                     </div>
                                 </button>
                             </div>
                         </transition>
                         <button @click="toggleMenu('color')" class="p-2 hmd:p-3 cursor-pointer rounded-xl transition-all relative z-10 hover:bg-slate-50 group">
                             <div class="w-4 h-4 hmd:w-5 hmd:h-5 rounded-full border-2 border-slate-200 shadow-sm group-hover:scale-110 transition-transform" :style="{ backgroundColor: currentColor }"></div>
                         </button>
                     </div>
                     ...
                     <div class="flex items-center bg-slate-100 rounded-xl p-0.5 space-x-0.5 shrink-0">
                         <button @click="undo" class="p-1.5 hmd:p-2.5 cursor-pointer rounded-lg text-slate-500 hover:bg-white hover:text-slate-700 hover:shadow-sm transition-all disabled:opacity-30 disabled:hover:bg-transparent" title="撤销 (Ctrl+Z)">
                             <RotateCcw class="w-3.5 h-3.5 hmd:w-4 hmd:h-4" />
                         </button>
                         <button @click="redo" class="p-1.5 hmd:p-2.5 cursor-pointer rounded-lg text-slate-500 hover:bg-white hover:text-slate-700 hover:shadow-sm transition-all disabled:opacity-30 disabled:hover:bg-transparent" title="重做 (Ctrl+Y)">
                             <RotateCw class="w-3.5 h-3.5 hmd:w-4 hmd:h-4" />
                         </button>
                     </div>

                     <!-- Fullscreen -->
                     <button @click="toggleFullscreen" class="p-2 hmd:p-3 rounded-xl cursor-pointer text-slate-400 hover:bg-slate-50 hover:text-slate-600 transition-colors shrink-0" title="全屏 (F)">
                     ...
                     <!-- Drag Handle -->
                     <div class="px-1.5 hmd:px-2 py-1 text-slate-300 hover:text-indigo-500 cursor-grab active:cursor-grabbing shrink-0 transition-colors group" title="按住拖拽工具栏">
                         <Grip class="w-4 h-4 hmd:w-5 hmd:h-5 group-hover:scale-110 group-active:scale-100 transition-transform" />
                     </div>
                 </div>

                 <!-- ================= MINIFIED TOOLBAR ================= -->
                 <div v-else key="collapsed" class="pointer-events-auto bg-white/80 backdrop-blur-sm shadow-[0_8px_32px_rgba(0,0,0,0.12)] border border-white/40 p-1 hmd:p-2 rounded-2xl flex items-center gap-1 hmd:gap-2">
                     ...
                     <!-- Active Tool View -->
                     <div class="relative shrink-0">
                         <button class="p-2 hmd:p-3 cursor-pointer rounded-xl bg-indigo-50 text-indigo-600 shadow-sm ring-1 ring-indigo-200 cursor-default">
                             <MousePointer2 v-if="currentTool === 'cursor'" class="w-4 h-4 hmd:w-5 hmd:h-5" />
                             <Pencil v-else-if="currentTool === 'pen'" class="w-4 h-4 hmd:w-5 hmd:h-5" />
                             <Eraser v-else-if="currentTool === 'eraser'" class="w-4 h-4 hmd:w-5 hmd:h-5" />
                             <div v-else class="w-4 h-4 hmd:w-5 hmd:h-5 rounded-full" :style="{ backgroundColor: currentColor }"></div>
                         </button>
                     </div>
