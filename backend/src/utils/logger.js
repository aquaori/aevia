...
class Logger {
    ...
    static welcome() {
        const title = gradient(["#00d2ff", "#9d50bb", "#ff0061"])(
            " COLLABORATIVE WHITEBOARD ENGINE ",
        );
        const box = boxen(
            `${chalk.bold(title)}\n\n${chalk.dim("Build amazing things together in real-time.")}\n\n${chalk.blue("→ Documentation:")} ${chalk.underline("https://github.com/aquaori/aevia")}`,
            {
                padding: 1,
                margin: 1,
                borderStyle: "round",
    ...
    static info(msg, ...args) {
        console.log(
            `${this.getTime()} ${chalk.blue("ℹ")} ${chalk.blue(msg)}`,
            ...args,
        );
    }
    ...
    static success(msg, ...args) {
        console.log(
            `${this.getTime()} ${chalk.green("✔")} ${chalk.green(msg)}`,
            ...args,
        );
    }
    ...
    static warn(msg, ...args) {
        console.log(
            `${this.getTime()} ${chalk.yellow("⚠")} ${chalk.yellow(msg)}`,
            ...args,
        );
    }
    ...
    static error(msg, ...args) {
        console.log(
            `${this.getTime()} ${chalk.red("✖")} ${chalk.red(msg)}`,
            ...args,
        );
    }
    ...
    static wsEvent(action, userName, userId, roomId, detail = "") {
        ...
        switch (action) {
            case "joined":
                console.log(
                    `${time} ${chalk.green("➡️ ")} ${userDisplay} joined ${roomDisplay} ${chalk.gray(detail)}`,
                );
                break;
            case "left":
                console.log(
                    `${time} ${chalk.red("⬅️ ")} ${userDisplay} left ${roomDisplay}`,
                );
                break;
            case "undo":
                console.log(
                    `${time} ${chalk.yellow("↩️ ")} ${chalk.dim(`[${roomDisplay}]`)} ${userDisplay} undo ${detail}`,
                );
                break;
            case "redo":
                console.log(
                    `${time} ${chalk.blue("↪️ ")} ${chalk.dim(`[${roomDisplay}]`)} ${userDisplay} redo ${detail}`,
                );
                break;
            default:
                console.log(
                    `${time} 🔔 ${chalk.dim(`[${roomDisplay}]`)} ${userDisplay} ${action} ${detail}`,
                );
        }
    }
    ...
    static cmd(type, cmdId) {
        ...
        console.log(
            `${this.getTime()} ${icon} ${chalk.dim("Received:")} ${chalk.yellow(type)} | ${chalk.gray(cmdId)}`,
        );
    }
}
